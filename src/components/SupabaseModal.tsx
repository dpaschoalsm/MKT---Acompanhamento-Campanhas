...
export const SupabaseModal: React.FC<SupabaseModalProps> = ({
  ...
  const handleUploadCurrentData = async () => {
    ...
    setUploadSuccess(false);
    try {
      const tasksOk = await upsertTasksBatchToSupabase(tasks);
      const campaignsOk = await upsertCampaignsBatchToSupabase(campaigns);

      if (tasksOk && campaignsOk) {
        setUploadSuccess(true);
        if (onSyncComplete) onSyncComplete();
      } else {
        alert('Houve uma falha ao enviar os dados. Verifique se executou o script SQL no Supabase para criar as tabelas.');
      }
    } catch (e: any) {
      console.error(e);
          ...
          <div className="p-6 space-y-5">
            ...
            {isConfigured && (
              <div className="bg-neutral-50 border border-neutral-200 rounded-lg p-4 space-y-3">
                ...
                {uploadSuccess && (
                  <div className="text-[11px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 p-2 rounded flex items-center gap-1.5">
